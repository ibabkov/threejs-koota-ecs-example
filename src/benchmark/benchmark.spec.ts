...
function computeFpsStats(values: number[]) {
  ...
  const avg = THREE.MathUtils.clamp(sum / values.length, 0, 120);
  const min = THREE.MathUtils.clamp(Math.min(...values), 0, 120);
  const max = THREE.MathUtils.clamp(Math.max(...values), 0, 120);
